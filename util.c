...
bool stratum_getjob_wildkeccak(struct stratum_ctx *sctx, struct work *work, struct scratchpad_hi current_scratchpad_hi) {
	char s[128];
	...
	free(prevhash);
	 ...
	 applog(LOG_INFO, "Getting job from stratum....");
	  ...
	  applog(LOG_DEBUG, "Getting job received line");


	json_t * val = JSON_LOADS(sret, &err);
	free(sret);
	if (!val) {
		applog(LOG_ERR, "JSON decode rpc2_getjob response failed(%d): %s", err.line, err.text);
		goto out;
	}
	
	bool ret = work_decode(json_object_get(val, "result"), work);
